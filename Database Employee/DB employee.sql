...
select e.Nom, e.prenom, e.id_dept, e.id_ville from Employee e
where e.id_region = (select v.id_region from Ville v where v.id_ville = e.id_ville)
SELECT Tel from employee where (Tel like '06%')
...
Nom varchar(20),
Prenom varchar(20),
Tel varchar(17) unique ,--3
Email varchar(100),
DateN date,
Date_Emb date,--6
sexe char(1),
Salaire Money default 0,
Adress varchar(100) default null, --9
...
id_ville int constraint FK_Employee_Ville Foreign key references Ville (ID_VILLE) ,--12
id_dept int constraint FK_Employee_Dpt Foreign key references Departement (ID_DEPT) on delete cascade on update cascade,
id_poste int constraint FK_Employee_poste Foreign key references Poste(ID_POSTE) ,--14
Photo image ,
Age int constraint ck_age_EMP check(Age > 18 )--16
)
